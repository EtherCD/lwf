...
export default class LWFLexer {
	...
	private text() {
		...
		let current = this.peek(0);
		while (true) {
			if (current === "," || current === "]" || current === "[") if (!buffer.includes("(") || (buffer.includes("(") && buffer.includes(")"))) break;
			buffer += current;
			current = this.next();
		}
		if (buffer === "true" || buffer === "false") this.addToken(types.LexerTokenType.BOOLEAN, buffer === "true" ? true : false);
